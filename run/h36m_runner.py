...
from datas import H36MMotionDataset, get_dct_matrix, reverse_dct_torch, define_actions, define_actions_cmu
...
def l1_loss_test(target, pred):
    super_result = torch.abs(pred - target)
    # super_result = torch.sum(super_result)  # reduction="none"
    reduced = super_result.mean()
    return reduced
...
class H36MRunner():
        ...
        train_dataset = H36MMotionDataset(self.cfg.base_data_dir, actions="all", mode_name="train", input_n=self.cfg.input_n, output_n=self.cfg.output_n,
                                      dct_used=self.cfg.dct_n, split=0, sample_rate=2,
                                          down_key=[('p22', 'p12', self.cfg.Index2212),
                                              ...
                                              ('p7', 'p4', self.cfg.Index74)], test_manner=self.cfg.test_manner, global_max=0, global_min=0, device=self.cfg.device)
        print("train data shape {}".format(len(train_dataset)))
        ...
        for act in define_actions("all"):
            test_dataset = H36MMotionDataset(self.cfg.base_data_dir, actions=act, mode_name="test", input_n=self.cfg.input_n, output_n=self.cfg.output_n,
                                      dct_used=self.cfg.dct_n, split=1, sample_rate=2,
                                          down_key=[('p22', 'p12', self.cfg.Index2212),
                                              ('p12', 'p7', self.cfg.Index127),
                                              ('p7', 'p4', self.cfg.Index74)], test_manner=self.cfg.test_manner, global_max=self.global_max, global_min=self.global_min, device=self.cfg.device)
            ...
            self.test_loader[act] = DataLoader(
                ...
                num_workers=self.cfg.num_works,
                pin_memory=True)
            print(">>> test {} data {}".format(act, len(test_dataset)))
    ...
    def test(self, epoch=0):
        ...
        for act_idx, act in enumerate(define_actions("all")):
            ...
            for i, (inputs, gts) in enumerate(self.train_loader):
                b, cv, t_len = inputs[0].shape
                for k in inputs:
                    inputs[k] = inputs[k].float().cuda(non_blocking=True, device=self.cfg.device)
                    gts[k] = gts[k].float().cuda(non_blocking=True, device=self.cfg.device)
        ...
        for epoch in range(self.start_epoch, self.cfg.n_epoch + 1):
            ...
            if epoch % 1 == 0:
                ...
                if np.mean(loss_l2_test) < self.best_accuracy:
                    ...
                    self.save(
                        os.path.join(self.cfg.ckpt_dir, "models", '{}_best_epoch{}_{}.pth'.format(self.cfg.exp_name, epoch,
                                                                     np.mean(loss_l2_test))), epoch, self.best_accuracy,
                        np.mean(loss_l2_test))

                self.save(os.path.join(self.cfg.ckpt_dir, "models",
                                       '{}_last{}_{}.pth'.format(self.cfg.exp_name, epoch, np.mean(loss_l2_test))),
